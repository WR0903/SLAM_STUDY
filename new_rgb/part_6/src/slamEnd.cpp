...
int main( int argc, char** argv )
{
    ...
    CAMERA_INTRINSIC_PARAMETERS camera = getDefaultCamera();//获取相机的内参
    computeKeyPointsAndDesp( lastFrame);//计算特征点和描述子
    ...
    *******************************/
    ...
    globalOptimizer.save("/home/wr/SLAM_STUDY/new_rgb/part_5/data/result_before.g2o");
    globalOptimizer.initializeOptimization();
    globalOptimizer.optimize( 100 ); //可以指定优化步数
    globalOptimizer.save( "/home/wr/SLAM_STUDY/new_rgb/part_5/data/result_after.g2o" );
...
double normofTransform( cv::Mat rvec, cv::Mat tvec )
{
    return fabs(min(cv::norm(rvec), 2*M_PI-cv::norm(rvec)))+ fabs(cv::norm(tvec));//求浮点数的绝对值
}
