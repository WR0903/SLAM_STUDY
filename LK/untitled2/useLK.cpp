...
int main( int argc, char** argv )
{
    ...
    string rgb_file, depth_file, time_rgb, time_depth;//从associate.txt中读取rgb图像和时间，以及对应的深度图像和时间
    ...
    for ( int index=0; index<100; index++ )
    {
        ...
        color = cv::imread( path_to_dataset+"/"+rgb_file );//读取rgb图像
        depth = cv::imread( path_to_dataset+"/"+depth_file, -1 );//读取深度图像
        if (index ==0 )
        {
            // 对第一帧提取FAST特征点
            vector<KeyPoint> kps;
        ...
        if ( color.data==nullptr || depth.data==nullptr )
            continue;//有些在associate.txt列出了，但没有对应的rgb和深度图像
        // 对其他帧用LK跟踪特征点
